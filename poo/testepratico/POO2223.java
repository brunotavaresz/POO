		...
		*/
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				Client client = campingService.getClient(Integer.parseInt(line[0]));
				LocalDate startDate = LocalDate.parse(line[1]);
				int duration = Integer.parseInt(line[2]);
				SpaceType spaceType = SpaceType.valueOf(line[3]);
				int[] dimensions = Arrays.stream(line[4].split("x")).mapToInt(Integer::parseInt).toArray();
